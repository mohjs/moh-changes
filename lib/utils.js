...
exports.fetchRepoURL = () => new Promise((resolve, reject) => {
  ...
  if (fs.existsSync(process.cwd() + '/.git')) {
    exec(getRepoURL, (err, stdout, stderr) => {
      ...
      emitter.emit('CHECK_REPO', 'succeed', 'Success get repo info')
      return resolve(transRepoInfo(transferUrl(stdout)))
    })
  } else {
    emitter.emit('CHECK_REPO', 'fail', 'Unexpected error happens')
...
exports.generateChangesInfo = () => {
  ...
  return Promise.all([
    ...
    emitter.emit('FETCH_INFO', 'succeed', 'Success fetch release and PR info')
    ...
    emitter.emit('FETCH_INFO', 'fail', 'Unexpected error during fetch')
...
exports.generateLog = changes => {
  fs.writeFile('./CHANGELOG.MD', j2m(changes).join('\n\n'), err => {
    if (err) {
      emitter.emit('GENERATE_LOG', 'fail', 'Failed generate Changelog')
    }
    emitter.emit('GENERATE_LOG', 'succeed', 'Success generate Changelog')
  })
}
