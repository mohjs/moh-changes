...
exports.generateExistInfo = () => new Promise((resolve, reject) => {  
  ...
  if (fs.existsSync(changePath)) {
    fs.readFile(changePath, 'UTF-8', (err, data) => {
      ...
      const currentInfo = m2j(data)

      emitter.emit('READ_INFO', 'succeed', ' Load current Changelog succeed')
      return resolve(currentInfo)
    })
  } else {
    emitter.emit('READ_INFO', 'succeed', ' No Changelog file, will generate one')
    return resolve(null)
  }
})

exports.generateChangesInfo = currentInfo => {
...
exports.generateLog = ({ currentInfo, changes }) => {
  fs.writeFile('./CHANGELOG.md', j2m({ currentInfo, changes }).join('\n\n'), err => {
    if (err) {
